<script lang="ts">
    ...
    let dragStartPos = { x: 0, y: 0 }; // 최초 클릭 위치 저장용

    let innerHeight = window.innerHeight;

    // Reactive statement to check if we are near the bottom of the screen.
    // The vertical toolbar is about 500px tall. We flip to horizontal
    // early so the user has plenty of room.
    $: isNearBottom = currentPos.y > innerHeight - 650;
    ...
    onMount(() => {
        // Init state: Center the toolbar initially so it's always visible on multi-monitor setups
        currentPos = {
            x: Math.max(20, (window.innerWidth / 2) - 30),
